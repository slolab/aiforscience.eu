...
| [FDA discussion paper on generative AI-enabled medical devices](fda-genai-medical-devices-2026.md) | US FDA, Center for Devices and Radiological Health | August 2026 | Out-of-domain regulatory discussion paper with no legal force; grounds BP08 on evaluating the deployed system against prespecified, task-scoped benchmarks with independent adjudication (also for an LLM judge) and staged confirmation in use; grounds BP09, BP04, BP03, BP10 on oversight checkpoints before irreversible actions, change control for third-party model updates, and injection through tool outputs. |
