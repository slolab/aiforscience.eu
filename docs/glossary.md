...
Interface { #interface }
:   A component added to an agent's [harness](#harness) so it can reach a tool, data source, or action outside the model, for example an MCP server, a skill, or a plugin.
...
Layer
:   The level a practice addresses.
    Operational: daily work with agents.
    Method: how studies and pipelines are designed.
    Ecosystem: institutions, infrastructure, and policy.
